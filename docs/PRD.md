# PRD — Visor360 (CCI Sistema de Gestão Analítica)
...
>
> Este sistema é **exclusivamente de análise e visualização de dados**. De todos os ~97 endpoints GET disponíveis na API Quality Automação, o Visor360 utilizará **SOMENTE requisições HTTP GET**. Em **nenhuma hipótese** o sistema deve realizar chamadas POST, PUT, DELETE, PATCH ou qualquer outro método que envie, altere, crie ou exclua dados. A **única exceção** é o endpoint de autenticação (login), que requer POST para envio de credenciais. Após autenticado, **100% das chamadas à API devem ser GET**.
...
| Atributo | Valor |
|---|---|
| **Nome** | Visor360 — CCI Sistema de Gestão Analítica |
