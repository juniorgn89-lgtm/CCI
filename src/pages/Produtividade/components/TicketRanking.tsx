  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-900">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Ranking por Ticket Medio</h3>
          <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">Top {displayData.length} vendedores por ticket medio</p>
        </div>
        {data.length > 0 && (
          <div className="hidden text-right sm:block">
            <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Maior ticket</p>
            <p className="text-sm font-bold text-gray-900 dark:text-gray-100">{formatCurrency(data[0].ticketMedio)}</p>
          </div>
        )}
      </div>
      <ResponsiveContainer width="100%" height={Math.max(300, displayData.length * 40)}>
        <BarChart data={displayData} layout="vertical" margin={{ left: 120 }}>
          ...
          />
          <Bar dataKey="ticketMedio" radius={[0, 6, 6, 0]}>
            {displayData.map((_, index) => (
              <Cell
                key={index}
                fill={index < 3 ? MEDAL_COLORS[index] : CHART_COLORS[2]}
