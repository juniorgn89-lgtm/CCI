...
/** Detalhe do frentista por produto — espelha o relatório de Abastecimento do
 *  webPosto (litros, preço médio, faturamento, nº de abastecimentos por produto). */
const FrentistaDetalheModal = ({ open, onClose, nome, codigo, abastecimentos }: FrentistaDetalheModalProps) => {
  const { linhas, totLitros, totFat, totAbast } = useMemo(() => {
    const map = new Map<string, ProdutoLinha>()
    for (const a of abastecimentos) {
      if (a.frentistaCodigo !== codigo) continue
      const nomeProd = a.produtoNome || `Produto ${a.produtoCodigo}`
      const prev = map.get(nomeProd) ?? { produto: nomeProd, litros: 0, faturamento: 0, abastecimentos: 0 }
      prev.litros += a.litros
      prev.faturamento += a.valorTotal
      prev.abastecimentos += 1
      map.set(nomeProd, prev)
    }
    const linhas = Array.from(map.values()).sort((a, b) => b.faturamento - a.faturamento)
    return {
      linhas,
      totLitros: linhas.reduce((s, l) => s + l.litros, 0),
      totFat: linhas.reduce((s, l) => s + l.faturamento, 0),
      totAbast: linhas.reduce((s, l) => s + l.abastecimentos, 0),
    }
          ...
          {linhas.length === 0 ? (
            ...
            <table className="mt-3 w-full text-xs">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {linhas.map((l) => (
                  <tr key={l.produto} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
