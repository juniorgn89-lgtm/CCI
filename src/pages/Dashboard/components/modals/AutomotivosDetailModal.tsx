...
interface AutomotivosDetailModalProps {
  open: boolean
  onClose: () => void
  dataInicial: string
  dataFinal: string
...
const PRODUTOS_AUTO = [
  'Lubrificante Mineral 1L',
  'Lubrificante Sintético 1L',
  'ARLA 32 — 20L',
  'Palheta Limpador',
  'Filtro de Ar',
  'Filtro de Óleo',
  'Aditivo Radiador',
  'Limpa Para-Brisa 500ml',
  'Pano Microfibra',
  'Cera Automotiva',
]
...
const AutomotivosDetailModal = ({
  ...
  const data = useMemo(() => {
    ...
    const dailyChart = generateDailyEvolution('automotivos', dates, totalFaturamento)

    const fatPerRow = distributeAcrossRows('auto-fat', PRODUTOS_AUTO.length, totalFaturamento)
    const qtdPerRow = distributeAcrossRows('auto-qtd', PRODUTOS_AUTO.length, qtdVendas)
    const marginPerRow = distributeAcrossRows('auto-mar', PRODUTOS_AUTO.length, totalLucroBruto)

    const rows = PRODUTOS_AUTO.map((nome, i) => {
  ...
  const handleRowClick = (row: typeof data.rows[number]) => {
    setDrilldown({
      ...
      dailyRows: buildDrilldownRows(`auto-${row.nome}`, data.dates, row.qtd, row.faturamento, row.margem),
    })
  }
      ...
      <Dialog open={open} onOpenChange={(o) => { if (!o) onClose() }}>
        <DialogContent className="flex max-h-[92vh] w-[96vw] max-w-3xl flex-col gap-4 overflow-hidden">
          <DialogHeader>
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <DialogTitle className="text-amber-700 dark:text-amber-300">Automotivos</DialogTitle>
              ...
              <tbody>
                {data.rows.map((row) => (
                  <tr
                    ...
                    className="cursor-pointer border-b border-gray-100 text-gray-800 transition-colors last:border-b-0 hover:bg-amber-50/50 dark:border-gray-800 dark:text-gray-200 dark:hover:bg-amber-900/10"
                  >
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Exibindo {data.rows.length} produtos · Faturamento total: {formatCurrencyInt(totalFaturamento)} · Margem média: {fmtPct(margemPct)}
          </p>
        </DialogContent>
      </Dialog>
...
export default AutomotivosDetailModal
