              ...
              {ordenados.map((p) => {
                ...
                return (
                  <tr key={p.empresaCodigo} className="hover:bg-gray-50 dark:hover:bg-gray-800/40">
                    ...
                    <td className="px-4 py-2.5 text-right tabular-nums text-gray-500 dark:text-gray-400">{part.toFixed(2).replace('.', ',')}%</td>
                    ...
                    <td className={cn('px-4 py-2.5 text-right font-medium tabular-nums', competitivo ? TONE_TEXT.emerald : caro ? TONE_TEXT.red : 'text-gray-400')}>
                      {diff >= 0 ? '+' : ''}{(diff * 100).toFixed(2).replace('.', ',')}%
                    </td>
                    {stats.showMargem && (
                      <td className="px-4 py-2.5 text-right tabular-nums text-gray-700 dark:text-gray-300">{p.comCustoPct >= 50 ? `${p.margem.toFixed(2).replace('.', ',')}%` : '—'}</td>
                    )}
                    <td className="px-4 py-2.5">
                      <div className="flex items-center justify-end gap-2">
