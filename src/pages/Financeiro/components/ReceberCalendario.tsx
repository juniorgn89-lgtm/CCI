...
/**
 * Calendário de recebimento — o que a rede TEM A RECEBER, por dia de vencimento.
 * 1 linha por dia (seg→dom) da semana ativa, com navegação de semana. Dias no
 * passado com título em aberto = vencido; no futuro = a vencer. Só dado real do
 * /TITULO_RECEBER (títulos em aberto). Fecho com Semana (subtotal) + Carteira (tudo).
 */
...
const ReceberCalendario = ({ data }: { data: ReceivableRow[] }) => {
  ...
  const carteiraTotal = useMemo(() => data.reduce((s, r) => (r.pendente ? s + r.valor : s), 0), [data])

  // Só as semanas que TÊM título a receber (+ a de hoje) — sem semanas vazias.
  const weeks = useMemo(() => {
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50/60 text-left text-[10px] uppercase tracking-wide text-gray-500 dark:border-gray-800 dark:bg-gray-900/50 dark:text-gray-400">
              ...
              <th className="px-2 py-2 font-medium">A receber</th>
          ...
          <tbody>
            ...
            {/* Fecho: subtotal da semana + carteira total */}
            ...
            <tr className="bg-gray-50/60 text-[12px] text-gray-500 dark:bg-gray-900/40 dark:text-gray-400">
              <td className="px-4 py-2" colSpan={4}>Carteira em aberto (todos os vencimentos)</td>
              <td className="px-4 py-2 tabular-nums font-semibold text-gray-700 dark:text-gray-300">{formatCurrencyInt(carteiraTotal)}</td>
              <td />
            </tr>
          </tbody>
        </table>
...
export default ReceberCalendario
