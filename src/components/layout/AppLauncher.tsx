    ...
    <>
      <DropdownMenu>
        ...
        >
          ...
          <div className="rounded-2xl bg-gray-50 p-1.5 dark:bg-white/[0.05]">
            <div className="grid grid-cols-3">
              {apps.map((app) => {
                ...
                const base = 'flex flex-col items-center rounded-2xl px-1 pb-2.5 pt-3.5 text-center transition-colors'
                ...
                return (
                  <div key={app.id} className={cn(base, 'hover:bg-black/[0.05] dark:hover:bg-white/10')}>
                    <a href={app.url} target="_blank" rel="noopener noreferrer" title={app.descricao} className="flex w-full flex-col items-center">
                      {icone}
                      {nome}
                    </a>
                    {app.pwa && (
          ...
          {/* Instalar o app atual neste aparelho (some quando já está instalado) */}
          {!instalado && (
            <button
              type="button"
              onClick={() => setInstallOpen(true)}
              className="mt-2 flex w-full items-center justify-center gap-1.5 rounded-xl px-3 py-2 text-[12px] font-semibold text-[#0F766E] transition-colors hover:bg-gray-50 dark:text-[#14b8a6] dark:hover:bg-white/5"
            >
